...
import AudioStreamMeter from "./AudioMeter";
...
export default class AudioStreamMeterVertecal extends AudioStreamMeter {
  renderAudioMeter() {
    ...
    this.canvas.width =
      channelHeight * this.levels.length + (this.levels.length - 1);
    ...
    for (let channel = 0; channel < this.levels.length; channel++) {
      ...
      if (Number.isFinite(this.target[channel])) {
        this.value[channel] +=
          (this.target[channel] - this.value[channel]) * 0.1;
      }
      ...
      this.context.fillRect(
        x,
        this.canvas.height - this.peak[channel],
        channelHeight,
        1
      );
    }
